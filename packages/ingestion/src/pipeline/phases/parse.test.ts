...
describe("parsePhase (cache-hit timing — W2-E.4)", () => {
  // This guard quantifies the warm-cache win that E.1 + E.2 + E.4 promise:
  // once every file has a valid parse-cache entry, re-running the parse
  // phase must be materially faster than the cold baseline. We assert a
  // loose "<=80% of cold" ceiling so CI variance doesn't cause flakes.
  //
...
describe("parsePhase (cache-hit stats — W2-E.4)", () => {
  let repo: string;
