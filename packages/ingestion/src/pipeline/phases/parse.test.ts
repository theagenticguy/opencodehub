...
describe("parsePhase (HAS_METHOD + HAS_PROPERTY emission)", () => {
  let repo: string;

  beforeEach(async () => {
    repo = await mkdtemp(path.join(tmpdir(), "och-parse-hasmethod-"));
  });
  ...
  it("emits HAS_METHOD for each method and HAS_PROPERTY for each property", async () => {
    // Fixture: one class with two methods and one class-body property.
    // Python is used here because the TS provider does not currently emit
    // `definition.property` captures for class fields (see ts-shared.ts
    // `TS_DEF_KIND_MAP`), so a `.ts` fixture would exercise HAS_METHOD only.
    // Python extractors populate Property for both typed and untyped class
    // attributes, giving us parity coverage on both edge types in one pass.
    await fs.writeFile(
      path.join(repo, "shape.py"),
      [
        "class Shape:",
        "    name: str = 'square'",
        "",
    ...
    const edges = [...graph.edges()];
    const nodes = [...graph.nodes()];
    const shape = nodes.find((n) => n.kind === "Class" && n.name === "Shape");
    assert.ok(shape, "expected Class Shape in graph");

    const hasMethodEdges = edges.filter((e) => e.type === "HAS_METHOD" && e.from === shape.id);
    const hasPropertyEdges = edges.filter((e) => e.type === "HAS_PROPERTY" && e.from === shape.id);

    assert.equal(hasMethodEdges.length, 2, "expected 2 HAS_METHOD edges (area, perimeter)");
    assert.equal(hasPropertyEdges.length, 1, "expected 1 HAS_PROPERTY edge (name)");

    // Contract: confidence 1.0 + reason "parse/ast" on every edge.
    for (const e of [...hasMethodEdges, ...hasPropertyEdges]) {
      assert.equal(e.confidence, 1, `edge ${e.type} should be confidence 1.0`);
      assert.equal(e.reason, "parse/ast", `edge ${e.type} should have reason "parse/ast"`);
    }

    // Each HAS_METHOD must point at a Method node; HAS_PROPERTY at a Property.
    const nodeById = new Map(nodes.map((n) => [n.id, n]));
    for (const e of hasMethodEdges) {
      assert.equal(nodeById.get(e.to)?.kind, "Method");
    }
    for (const e of hasPropertyEdges) {
      assert.equal(nodeById.get(e.to)?.kind, "Property");
    }
  });

  it("nested class: outer owns outer methods, inner owns inner methods; no duplicates", async () => {
    await fs.writeFile(
      path.join(repo, "nested.py"),
      [
        "class Outer:",
    ...
    const outer = nodes.find((n) => n.kind === "Class" && n.name === "Outer");
    const inner = nodes.find((n) => n.kind === "Class" && n.name === "Inner");
    assert.ok(outer, "expected Outer class");
    assert.ok(inner, "expected Inner class");

    const outerMethod = nodes.find((n) => n.kind === "Method" && n.name === "outer_method");
    const innerMethod = nodes.find((n) => n.kind === "Method" && n.name === "inner_method");
    assert.ok(outerMethod, "expected outer_method Method node");
    assert.ok(innerMethod, "expected inner_method Method node");

    const hasMethodEdges = edges.filter((e) => e.type === "HAS_METHOD");

    // Outer owns exactly outer_method (not inner_method).
    const outerOwns = hasMethodEdges.filter((e) => e.from === outer.id);
    assert.equal(outerOwns.length, 1, "Outer should own exactly 1 method");
    assert.equal(outerOwns[0]?.to, outerMethod.id, "Outer should own outer_method directly");

    // Inner owns exactly inner_method.
    const innerOwns = hasMethodEdges.filter((e) => e.from === inner.id);
    assert.equal(innerOwns.length, 1, "Inner should own exactly 1 method");
    assert.equal(innerOwns[0]?.to, innerMethod.id, "Inner should own inner_method");

    // No duplicate: Outer must NOT own Inner's method.
    const outerOwnsInner = hasMethodEdges.find(
      (e) => e.from === outer.id && e.to === innerMethod.id,
    );
    assert.equal(outerOwnsInner, undefined, "Outer must not double-claim Inner's method");
  });

  it("type-level: relation of type HAS_METHOD is accepted by KnowledgeGraph.addEdge", () => {
    // Compile-time guard: the union accepts HAS_METHOD/HAS_PROPERTY.
    const g = new KnowledgeGraph();
    const cls = {
      id: "Class#src/x.ts#X" as never,
      kind: "Class" as const,
      name: "X",
      filePath: "src/x.ts",
    };
    const m = {
      id: "Method#src/x.ts#X.m" as never,
    ...
    g.addNode(cls);
    g.addNode(m);
    g.addEdge({ from: cls.id, to: m.id, type: "HAS_METHOD", confidence: 1, reason: "parse/ast" });
    g.addEdge({
      from: cls.id,
      to: m.id,
      type: "HAS_PROPERTY",
    ...
    assert.deepEqual(types, ["HAS_METHOD", "HAS_PROPERTY"]);
  });
});
