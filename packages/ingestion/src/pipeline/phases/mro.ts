...
function runMro(ctx: PipelineContext): MroOutput {
  // ---- Stream L: incremental carry-forward. -----------------------------
  //
  ...
  for (const id of ownerIds) {
    ...
    // Stream L incremental gate: owners outside the closure keep their
    // prior-graph MRO edges (carried forward above). Correctness for the
    // in-closure case holds because incremental-scope already widened the
    // closure by one heritage hop.
