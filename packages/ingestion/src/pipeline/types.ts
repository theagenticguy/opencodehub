 ...
 *
 * Wave 5 ships the runner plus the first three phases (scan, structure,
 * parse). Later waves add markdown, routes, tools, orm, crossFile, mro,
 * communities, and processes without touching this file.
 */
 ...
 * phase (W2-E.3) to compute the import-closure walk. We intentionally keep
 * this narrower than a full {@link KnowledgeGraph} so callers can materialise
...
export interface PreviousGraph {
  ...
  /**
   * Full prior-run node set (Stream L, W2-E.4). When present, Stream L phases
   * (crossFile/mro/communities/processes) carry forward any non-closure node
   * from the prior run so incremental mode produces a byte-identical graph
   * hash vs a fresh full run at the same commit. `undefined` keeps the v1.0
   * passive contract where incremental-scope emits the closure hint but the
   * downstream phases re-compute from scratch.
   */
  readonly nodes?: readonly GraphNode[];
  /**
   * Full prior-run edge set (Stream L, W2-E.4). Paired with {@link nodes};
   * see its docstring for activation semantics. Stream L phases use this
   * snapshot to carry forward edges whose both endpoints lie outside the
   * closure unchanged. Edges whose endpoints are now missing from the
   * current graph are silently dropped on carry-forward.
   */
