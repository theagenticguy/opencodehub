/**
 * Stream L — incremental determinism gate.
 *
 * Proves that `--full` and `--incremental` analyses of the same commit
 * produce byte-identical graph hashes. Without this test the four Stream L
 ...
 * edge snapshot here so Stream L consumers can carry forward non-closure
...
async function writeFixture(repo: string): Promise<void> {
  ...
  // heritage chain + cross-file calls ensure every Stream L consumer has
  // real work to do.
  await fs.writeFile(
...
describe("incremental-determinism", () => {
  ...
  it("Test C: incremental with no closure (prior graph same commit) equals full hash", async () => {
    ...
    try {
      ...
      // Second run: same files, same hashes → closure is empty; Stream L
  ...
  it("Test B: touch one file with no semantic change → incremental hash equals full hash", async () => {
    ...
    try {
      ...
      // hashing — that is a product decision pinned in Stream E.
