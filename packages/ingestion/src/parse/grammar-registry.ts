 ...
 * cache cleanly, satisfying the Stream E cache-key invariant.
 */
 ...
 * produces a new SHA, which is what the Stream E content-addressed parse
 * cache needs in its composite key.
 *
 * Returns `null` when:
 *   - the grammar package is not installed (e.g. W2-C.* languages whose
 *     provider track has not landed yet), OR
