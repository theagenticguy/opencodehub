...
test("registry: MRO strategies are assigned per the language family", () => {
  const expected: Readonly<Record<LanguageId, MroStrategyName>> = {
    ...
    go: "none",
    // W2-C.1 stubs — values locked from .erpaval/sessions/002/research-languages.yaml.
    c: "none",
    cpp: "c3",
    ruby: "c3",
    kotlin: "c3",
...
test("registry: every provider returns empty arrays for empty inputs", () => {
  // Real providers from W2-C.2/3/4 should still tolerate empty captures
  // gracefully — the pipeline feeds them zero-capture inputs for files
  // with no matches.
...
test("registry: W2-C languages pick the right heritage edge", () => {
