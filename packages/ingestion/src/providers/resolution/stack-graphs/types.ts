...
// The public upstream evaluator (Rust) supports far more; our types are
// intentionally narrow. All names and semantics were chosen independently
// from upstream source code — we model only what our Python rules need.

/**
 * Node roles in our simplified stack-graph. Each maps to one primitive
 * upstream attribute family, but we collapse push-scoped / pop-scoped into
 * `push`/`pop` here because Python's reference model doesn't require the
 * scope-stack machinery for the re-export cases we actually resolve.
 */
 ...
 * only the shapes our Python evaluator consults; other upstream rule kinds
