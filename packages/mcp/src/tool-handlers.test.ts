...
function makeFakeStore(data: FakeStoreData): DuckDbStore {
  const api = {
    ...
    query: async (
      ...
      // Analysis package: relation-type lookup
      if (text.startsWith("SELECT from_id, to_id, type FROM relations")) {
        const half = Math.floor(params.length / 2);
        const froms = new Set((params as string[]).slice(0, half));
        const tos = new Set((params as string[]).slice(half));
      ...
      // Context tool: name-based lookup
      if (text.startsWith("SELECT id, name, kind, file_path FROM nodes WHERE name = ?")) {
        const [name, kindMaybe, fileMaybe] = params as string[];
        return data.nodes
      ...
      // Context tool: CALLS neighbourhood
      if (text.includes("r.type = 'CALLS'") && text.includes("r.to_id = ?")) {
        const id = params[0];
        return data.relations
          .filter((r) => r["type"] === "CALLS" && r["to_id"] === id)
          .map((r) => {
            const src = data.nodes.find((n) => n["id"] === r["from_id"]) ?? {};
            return projectNode(src);
          });
      }
      if (text.includes("r.type = 'CALLS'") && text.includes("r.from_id = ?")) {
        const id = params[0];
        return data.relations
          .filter((r) => r["type"] === "CALLS" && r["from_id"] === id)
          .map((r) => {
            const src = data.nodes.find((n) => n["id"] === r["to_id"]) ?? {};
            return projectNode(src);
          });
      }
      ...
      // Context tool: File-node lookup by file_path (cochange resolver).
      if (text.startsWith("SELECT id FROM nodes WHERE kind = 'File' AND file_path = ?")) {
        const p = params[0];
        return data.nodes
          .filter((n) => n["kind"] === "File" && n["file_path"] === p)
          .map((n) => ({ id: n["id"] }));
      }
      // Context tool: COCHANGES top-N query (union of from/to directions).
      if (text.includes("r.type = 'COCHANGES'") && text.includes("r.from_id = ? OR r.to_id = ?")) {
        const fileId = params[0];
        return data.relations
          .filter(
            (r) => r["type"] === "COCHANGES" && (r["from_id"] === fileId || r["to_id"] === fileId),
          )
          .map((r) => {
            const partnerId = r["from_id"] === fileId ? String(r["to_id"]) : String(r["from_id"]);
...
test("context surfaces top-10 COCHANGES partners for a File target", async () => {
  await withTestHarness(
    {
      nodes: [
        { id: "File:src/a.ts:src/a.ts", name: "src/a.ts", kind: "File", file_path: "src/a.ts" },
        { id: "File:src/b.ts:src/b.ts", name: "b.ts", kind: "File", file_path: "src/b.ts" },
        { id: "File:src/c.ts:src/c.ts", name: "c.ts", kind: "File", file_path: "src/c.ts" },
      ],
      relations: [
        {
          id: "E:cc:ab",
          from_id: "File:src/a.ts:src/a.ts",
          to_id: "File:src/b.ts:src/b.ts",
          type: "COCHANGES",
          confidence: 1,
          reason: JSON.stringify({ hops: 1, coCommitCount: 5 }),
        },
        {
          id: "E:cc:ac",
          from_id: "File:src/a.ts:src/a.ts",
          to_id: "File:src/c.ts:src/c.ts",
          type: "COCHANGES",
          confidence: 0.25,
          reason: JSON.stringify({ hops: 2, coCommitCount: 1 }),
        },
      ],
    },
    async (ctx, server) => {
      ...
      const sc = result.structuredContent as {
        target: { id: string };
        cochangeTop10: Array<{ fileId: string; filePath: string; score: number; hops: 1 | 2 }>;
      };
      assert.equal(sc.target.id, "File:src/a.ts:src/a.ts");
      assert.equal(sc.cochangeTop10.length, 2);
      // Highest-confidence partner ranks first.
      assert.equal(sc.cochangeTop10[0]?.filePath, "src/b.ts");
      assert.equal(sc.cochangeTop10[0]?.hops, 1);
      assert.equal(sc.cochangeTop10[0]?.score, 1);
      assert.equal(sc.cochangeTop10[1]?.filePath, "src/c.ts");
      assert.equal(sc.cochangeTop10[1]?.hops, 2);
    },
  );
});

test("context resolves co-change partners for a symbol via its File", async () => {
  await withTestHarness(
    {
      nodes: [
        { id: "File:src/a.ts:src/a.ts", name: "src/a.ts", kind: "File", file_path: "src/a.ts" },
        { id: "File:src/b.ts:src/b.ts", name: "src/b.ts", kind: "File", file_path: "src/b.ts" },
        { id: "F:foo", name: "foo", kind: "Function", file_path: "src/a.ts" },
      ],
      relations: [
        {
          id: "E:cc:ab",
          ...
          type: "COCHANGES",
          confidence: 0.75,
          reason: JSON.stringify({ hops: 1, coCommitCount: 3 }),
        },
      ],
    },
    async (ctx, server) => {
      ...
      const sc = result.structuredContent as {
        target: { id: string };
        cochangeTop10: Array<{ filePath: string }>;
      };
      assert.equal(sc.target.id, "F:foo");
      assert.equal(sc.cochangeTop10.length, 1);
      assert.equal(sc.cochangeTop10[0]?.filePath, "src/b.ts");
    },
  );
});
...
test("impact drives the analysis package and groups by depth", async () => {
  await withTestHarness(
    ...
    async (ctx, server) => {
      ...
      const sc = result.structuredContent as {
        risk: string;
        byDepth: Array<{ depth: number; nodes: unknown[] }>;
        ambiguous: boolean;
      };
      assert.equal(sc.ambiguous, false);
      assert.ok(Array.isArray(sc.byDepth));
