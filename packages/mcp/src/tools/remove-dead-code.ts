...
export async function runRemoveDeadCode(
  ctx: RemoveDeadCodeContext,
  args: RemoveDeadCodeArgs,
): Promise<ToolResult> {
  ...
  const call = await withStore(ctx, args.repo, async (store, resolved) => {
    try {
      ...
      // confirmation — the tool refuses to persist deletions without an explicit opt-in.
      if (!dryRun && !apply) {
        return toolError(
          "INVALID_INPUT",
      ...
      if (applied) {
        next.push("re-index the repo with `codehub analyze` so the graph reflects the removals");
      }
      if (totalDeletions === 0) {
...
export function registerRemoveDeadCodeTool(server: McpServer, ctx: RemoveDeadCodeContext): void {
  server.registerTool(
    ...
    async (args) => fromToolResult(await runRemoveDeadCode(ctx, args)),
  );
}
