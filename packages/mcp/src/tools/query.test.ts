...
function makeFakeStore(opts: FakeStoreOptions): FakeStoreHandle {
  ...
  const impl = {
    ...
    // ITemporalStore.exec — the `query` path issues no ad-hoc SQL, so the
    // fake returns an empty result set for anything it's handed.
    exec: async (
      _sql: string,
      _params: readonly SqlParam[] = [],
    ): Promise<readonly Record<string, unknown>[]> => [],
    // Cochange surfaces — unused by `query`, but required to satisfy the
    // full IGraphStore / ITemporalStore interfaces.
