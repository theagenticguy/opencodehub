 ...
 * (W2-I5) from per-ecosystem manifest parsers. Every node carries
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return withStore(ctx, args.repo, async (store, resolved) => {
        try {
          // The storage layer has dedicated columns for Dependency nodes
          // (W1-STORE): `version`, `license`, `lockfile_source`,
          // `ecosystem`. We read them directly instead of unpacking a
          // generic properties blob.
