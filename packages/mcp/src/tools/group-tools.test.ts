...
test("group_query fans out BM25 + RRF with Stream J response shape", async () => {
  await withTestHarness(
    [
      {
        name: "alpha",
...
// ---------------------------------------------------------------------------
// Stream J acceptance — two-repo same-name isolation + determinism.
// ---------------------------------------------------------------------------

/**
 * Fixture: two repos `alpha` and `bravo` each define a symbol named `foo`
 * at a different path. Exercises the three Stream J invariants:
...
test("[Stream J] query(repo:bravo) returns only bravo's symbol — physical repo isolation", async () => {
  await withTestHarness(
    SAME_NAME_REPOS,
...
test("[Stream J] query without repo arg returns AMBIGUOUS_REPO when >1 repo registered", async () => {
...
test("[Stream J] group_query returns both same-name symbols tagged with _repo", async () => {
  await withTestHarness(
    SAME_NAME_REPOS,
    [{ name: "pair", repos: ["alpha", "bravo"] }],
    async (ctx, server) => {
...
test("[Stream J] group_query is deterministic across 3 successive runs (byte-equal structured JSON)", async () => {
  await withTestHarness(
    SAME_NAME_REPOS,
    [{ name: "pair", repos: ["alpha", "bravo"] }],
    async (ctx, server) => {
