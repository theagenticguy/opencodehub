 ...
 * Float32Array per input. The same input MUST produce the byte-identical
 * output across repeat calls; this is the contract the graphHash CI gate
 * relies on. See .erpaval/sessions/002/research-embeddings.yaml §5 for the
 * exhaustive determinism rationale.
 *
 * The weights themselves are NOT downloaded here — W2-A.2 owns that code
 * path. If the weights are absent we throw {@link EmbedderNotSetupError} so
 * callers can degrade to BM25-only search gracefully.
 */
