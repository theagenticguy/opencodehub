...
class OpenCodeHubAgent:
    ...
    #
    # NOTE: ``risk_trends`` and ``verdict`` correspond to MCP tools that
    # are still in flight (W3-F.1 / W3-F.2). When the tool is not yet
    # registered the server returns an error envelope; ``_call`` folds
    # that into ``{"isError": True, ...}`` so the eval harness records a
