...
| "Bespoke temporal query (cochanges)" | `sql` | Read-only SQL against the temporal store (cochanges), 5s timeout. NOT the node/edge graph. | A typed tool covers it; or you need the graph — use Cypher (MCP `sql` `cypher` arg). |
