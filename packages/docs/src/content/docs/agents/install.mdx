...
- **Node** ≥24.15. The store is the built-in `node:sqlite` and parsing
  is `web-tree-sitter` (WASM): no native binding, no build step.
- **pnpm** 11 or newer.
- **git**.
...
- `.gitignore` — appends `.codehub/` so the local `store.sqlite` index
...
Storage is one always-present file under `.codehub/`: the whole index
lives in `store.sqlite` (WAL mode) via Node's built-in `node:sqlite`,
holding graph nodes, edges, embeddings, and the temporal tables. There
is no selection knob, no native storage binding, and no platform where
this fails for lack of a prebuilt. See
[ADR 0019](https://github.com/theagenticguy/opencodehub/blob/main/docs/adr/0019-single-file-sqlite-storage.md).
...
(WASM) parse runtime, the built-in `node:sqlite` store (checked with an
import plus a WAL round-trip), and which embedding backend is in effect
(SageMaker → HTTP → local ONNX, in that precedence). There is no native
storage binding to probe and parsing is WASM-only, so doctor reports OK
on any Node ≥24.15.
