...
export function buildGroupCommand(opts: GroupCommandOptions = {}): Command {
  ...
  root
    ...
    .action(async (name: string, subOpts: Record<string, unknown>) => {
 ...
 */
export async function runGroupSyncCmd(
  groupName: string,
  opts: GroupSyncOptions = {},
): Promise<GroupSyncResult> {
  const registryOpts = opts.home !== undefined ? { home: opts.home } : {};
  const group = await readGroup(groupName, registryOpts);
  if (!group) {
    throw new Error(
      `codehub group sync: group ${groupName} not found. Run \`codehub group create\` first.`,
    );
  }
  const registry = await readRegistry(registryOpts);
  const sortedRepos = [...group.repos].sort((a, b) =>
  ...
  const syncInputs: SyncRepoInput[] = [];
  const missing: string[] = [];
  for (const repo of sortedRepos) {
    const hit = registry[repo.name];
    if (!hit) {
  ...
  if (opts.now !== undefined) syncOpts.now = opts.now;
  ...
  if (opts.json) {
    ...
    if (missing.length > 0) {
      console.warn(
        `codehub group sync: skipped ${missing.length} unregistered repo(s): ${missing.join(", ")}`,
      );
    }
  }
