...
export interface FileNode extends NodeBase {
  ...
  // --- Temporal signals (Stream H.2). All optional; populated only when the
  ...
  // --- Ownership signals (Stream H.3 / H.5). All optional; emitted by the
  // `ownership` phase. ---
  /**
  ...
  /** Line-level coverage ratio in [0, 1] from the coverage overlay (Stream Q.2). */
  readonly coveragePercent?: number;
  /** Covered 1-based line numbers from the coverage overlay (Stream Q.2). */
...
export interface CommunityNode extends NodeBase {
  ...
  // --- Community-level ownership metrics (Stream H.4). ---
  /**
