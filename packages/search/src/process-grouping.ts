/**
 * Process-grouping stub (MVP).
 *
 * Real process detection (entrypoint walks, PROCESS_STEP edges) ships in the
 * Wave 7 processes phase. At MVP we simply emit one bucket per hit tagged
 * `"ungrouped"`, so the CLI and MCP server can call this function today
