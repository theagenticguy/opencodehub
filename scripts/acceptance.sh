#!/usr/bin/env bash
# scripts/acceptance.sh — MVP + v1.0 Definition-of-Done verifier.
#
# Runs every acceptance gate from .erpaval/sessions/001/plan.yaml → gates.mvp_done
# plus the v1.0 extensions from .erpaval/sessions/002/plan.yaml (W5-4).
#
...
# The verdict CLI is in-flight (W3-F.1). If neither the CLI subcommand nor
# the verdict MCP tool is present, skip.
...
if [ "$VERDICT_CLI_PRESENT" -eq 0 ] && [ "$VERDICT_MCP_PRESENT" -eq 0 ]; then
  skip "verdict command/tool not yet wired (W3-F.1 in progress)"
elif [ ! -f "$CLI" ]; then
